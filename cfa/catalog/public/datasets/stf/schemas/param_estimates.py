...
load_schema = pa.DataFrameSchema(
    columns={
        ...
        "parameter": pa.Column(
            str,
            pa.Check.isin(
                ["delay", "right_truncation", "generation_interval"]
            ),
        ),
        ...
        "value": pa.Column(
            ...
            pa.Check(
                lambda x: x.apply(
                    lambda arr: all(0 <= i <= 1 for i in arr)
                    and sum(arr) <= 1.01
                )
            ),
            nullable=True,
        ),  # Check if value is a list of numbers between 0 and 1
    }
)
...
def load_mock_data(output="pandas", size=10) -> pd.DataFrame | pl.DataFrame:
    ...
    df = df.assign(
        ...
        parameter=random.choices(
            ["delay", "right_truncation", "generation_interval"], k=size
        ),
        format=["PMF"] * size,
